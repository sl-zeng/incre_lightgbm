...
void Application::Train() {
  ...
  if (!config_.input_model.empty()){
    boosting_->MyTrain(config_.snapshot_freq, config_.output_model, config_.input_model, config_.num_iterations, config_.erase_pos);
  } else {
...
void Application::Predict() {
  if (config_.task == TaskType::KRefitTree) {
    ...
    Log::Info("%f seconds elapsed", std::chrono::duration<double,
              std::milli>(end_time - start_time) * 1e-3);
