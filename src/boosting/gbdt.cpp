...
void GBDT::Init(const Config* config, const Dataset* train_data, const ObjectiveFunction* objective_function,
  ...
  std::cout << "GBDT init : num_data = " << num_data_ << std::endl; 
...
void GBDT::MyTrain(int snapshot_freq, const std::string& model_output_path, const std::string& input_model_path, int num_iterations, int erase_pos) {
  bool is_finished = false;
  //for (int i = 0; i < num_iterations; i++){
    //models_.pop_back();
    //models_.erase(models_.begin());
  //}
  std::cout << "in MyTrain: erase_pos =" << erase_pos << std::endl;
  models_.erase(models_.begin() + erase_pos);
  auto start_time = std::chrono::steady_clock::now();
  for (int iter = 0; iter < config_->num_iterations && !is_finished; ++iter) {
    is_finished = TrainOneIter(nullptr, nullptr);
    if (!is_finished) {
...
const double* GBDT::GetTrainingScore(int64_t* out_len) {
  //std::cout << "enter GetTrainingScore " << std::endl;
  //std::cout << train_score_updater_->num_data() << std::endl;
  //std::cout << "finish test num_data " << std::endl;
  *out_len = static_cast<int64_t>(train_score_updater_->num_data()) * num_class_;
  //std::cout << "leave GetTrainingScore " << std::endl;
